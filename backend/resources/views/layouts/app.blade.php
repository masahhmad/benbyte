...
<body>
    ...
    <div id="app">
        @if (Route::currentRouteName() !== 'admin.view-kreator' && Route::currentRouteName() !== Auth::user()->role . '.blog' && Route::currentRouteName() !== Auth::user()->role . '.komentar' && Route::currentRouteName() !== 'kreator.konten-kelas' && Route::currentRouteName() !== 'user.registrasi-kreator' && Route::currentRouteName() !== 'user.masuk-kelasku')
            <div id="sidebar">
                <div class="sidebar-wrapper active">
                    ...
                    @elseif (Auth::user()->role == 'kreator')
                        @include('layouts.sidebars.kreators-sidebar')
                    @elseif (Auth::user()->role == 'user')
                        @include('layouts.sidebars.users-sidebar')
                    @endif

                @else
                    @include('layouts.sidebars.log-reg-sidebar')
                @endif
                </div>
            </div>
        @endif

        <div id="{{Route::currentRouteName() === 'admin.view-kreator' || Route::currentRouteName() === Auth::user()->role . '.blog' || Route::currentRouteName() === Auth::user()->role . '.komentar' || Route::currentRouteName() === 'kreator.konten-kelas' || Route::currentRouteName() === 'user.registrasi-kreator' || Route::currentRouteName() === 'user.masuk-kelasku' ? '' : 'main'}}" class='layout-navbar navbar-fixed'>
        
        @if (Auth::check())
            @if (Route::currentRouteName() === 'admin.view-kreator' || Route::currentRouteName() === Auth::user()->role . '.blog' || Route::currentRouteName() === Auth::user()->role . '.komentar' || Route::currentRouteName() === 'kreator.konten-kelas' || Route::currentRouteName() === 'user.registrasi-kreator' || Route::currentRouteName() === 'user.masuk-kelasku')
                @include('layouts.single-navbar')
            @else
        ...
        {{-- Comment --}}
        @if (Auth::user()->role !== 'admin')
            @if (Route::currentRouteName() === Auth::user()->role . '.home')
                <div class="position-fixed bottom-0 end-0 m-5">
                    <a href="{{route(auth::user()->role . '.komentar')}}" class="fs-1">
